---
layout: work
title: 이런 프로젝트를 참여했어요
slug: /work
items:
  - title: 2021-11 ~ 진행중 OSINT 기술 자문 (내용 비공개)
    image:
      src: /assets/img/work/nshc.png
      alt: nshc
    description: <br />OSINT 컨설팅 기술 자문 및 리서치 기여 <br /> (상세 내용 비공개)
  - title: 2021-11 ~ 진행중<a href="https://www.deficookie.com/"/> DeFiCookie</a>
    image:
      src: /assets/img/work/CookieBear.png
      alt: CookieBear
    description: <br />FE 설계 및 개발, PM <br /> ReactJs, NodeJs, MongoDB, AWS, Web3
  - title: 2021-04 ~ 2020-06 <a href="https://www.cointantei.com/"/> CoinTantei</a>
    image:
      src: /assets/img/work/cointantei.png
      alt: cointantei
    description: <br />UI 설계, 디자인, 개발, 배포 및 운영, 유지보수<br /> LightSail, ReactJs, NodeJs
  - title: 2020-08 ~ 2020-12 ODCNC
    image:
      src: /assets/img/work/odcnc.png
      alt: odcnc
    description: <br />데이터 파이프라인 유지보수 및 고도화, erp 세팅 <br /> python3
  - title: 2019-08 ~ 2019-11 내부 프로젝트 (내용 비공개)
    image:
      src: /assets/img/work/coc.png
      alt: coc
    description: <br />데이터 수집 서버 고도화, 취약점을 이용한 데이터 수집, <br />가공 및 보고 체계 자동화 설계 및 개발 (내용 비공개)<br /> Java , MySQL , JSP , Apache Tomcat , HTML5, CSS3, jQuery, javascript, python3
  - title: 2019-05 ~ 2019-08 내부 프로젝트 (내용 비공개)
    image:
      src: /assets/img/work/coc.png
      alt: coc
    description: <br />데이터 수집 서버 배포 및 운영, 데이터 가공 및 보고 체계 자동화 설계 및 개발 <br />(내용 비공개)<br /> Java , MySQL , JSP , Apache Tomcat , HTML5, CSS3, jQuery, javascript, python3
  - title: 2019-01 ~ 2019-04 내부 프로젝트 (내용 비공개)
    image:
      src: /assets/img/work/coc.png
      alt: coc
    description: <br />데이터 수집 서버 설계 및 개발, 데이터 대시보드 설계 및 개발 (내용 비공개)<br /> Java , MySQL , JSP , Apache Tomcat , HTML5, CSS3, jQuery, javascript
  - title: 2018-09 ~ 2018-12 내부 프로젝트 (내용 비공개)
    image:
      src: /assets/img/work/coc.png
      alt: coc
    description: <br />네트워크 장비를 이용한 네트워크 환경 설계 및 구성 (내용 비공개)<br /> router, switch, NAT
---

<br />
열심히 업로드와 수정 중이에요!
<br />
<br />

<!--

 Life Log data Set


- title: My Interest 1
    image:
      src: /assets/img/work/water.png
      alt: water
    description: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.



-->
